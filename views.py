...
def calculateMetrics(algorithm, predict, test_labels):
    ...
    score = 1/mean_absolute_error(np.asarray(test_labels), np.asarray(predict))
    rmse.append(mse_value)
    mae.append(score)

# Load and preprocess the water level dataset
water_bearing = pd.read_csv("Dataset/District_Statewise_Waterlevel.csv")
...
le = LabelEncoder()
water_bearing['Name of State'] = pd.Series(le.fit_transform(water_bearing['Name of State'].astype(str)))
le1 = LabelEncoder()
water_bearing['Name of District'] = pd.Series(le1.fit_transform(water_bearing['Name of District'].astype(str)))
...
water_bearing_X = scaler2.fit_transform(water_bearing_X)
...
# Build the model
model1 = Sequential()
model1.add(Dense(64, input_dim=input_dim, activation='relu'))  # First hidden layer
model1.add(Dropout(0.2))  # Dropout for regularization
model1.add(Dense(32, activation='relu'))  # Second hidden layer
model1.add(Dense(1, activation='linear'))  # Output layer for regression

# Compile the model
model1.compile(optimizer='adam', loss='mean_squared_error', metrics=['mae'])

# Train the model
model1.fit(X_train, y_train, epochs=100, batch_size=32, verbose=1, validation_split=0.1)
...
# Load and preprocess the current water dataset
...
# Normalize features and target for current water dataset
...
# Split the data
X_train, X_test, y_train, y_test = train_test_split(current_X, current_Y, test_size=0.05)

# Random Forest Regressor for current water
modell = RandomForestRegressor()
modell.fit(X_train, y_train.ravel())
...
calculateMetrics("Deep CNN ",predictions, y_test)

# Calculate metrics for CNN
#calculateMetrics("Deep CNN ", cnn_predictions, y_test)
...
def TrainModels(request):
    if request.method == 'GET':
        ...
        output += '</tr>'
        algorithms = [ 'Random Forest','Deep CNN']
...
def Visualization(request):
    if request.method == 'GET':
        ...
        df = pd.read_csv("Dataset/District_Statewise_Waterlevel.csv")
...
def FeedbackAction(request):
    if request.method == 'POST':
        ...
        if sentiment_dict['compound'] >= 0.05:
            ...
            label = "Neutral"
        db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'waterwell',charset='utf8')
...
def RegisterAction(request):
    if request.method == 'POST':
        ...
        status = 'none'
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'waterwell',charset='utf8')
        with con:
            cur = con.cursor()
